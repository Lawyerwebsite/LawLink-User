...
const BookAppointment = () => {
  ...
  const handleBooking = async () => {
    ...
    const paymentData = {
      amount: 1, // Example amount in INR, adjust accordingly
      currency: 'INR',
    ...
    try {
      ...
      if (orderResponse.data.success) {
        ...
        const options = {
          key: 'rzp_test_CYTmMpZvcD8YhS', // Replace with your Razorpay key
            ...
            {slots && slots.length > 0 ? (
              slots.map((slot, index) => (
                <button
                  ...
                  className={`w-full p-3 border border-gray-300 rounded-md ${
                    selectedSlot === slot ? 'bg-blue-500 text-white' : 'bg-white text-gray-700'
                  }`}
                >
                  {slot}
                </button>
